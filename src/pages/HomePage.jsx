...
function HomePage({ onNavigate, onPlanSelection, onPageLoad }) {
  ...
  const [canAnimate, setCanAnimate] = useState(false); // Control when animations can start
  const [heroSlideIn, setHeroSlideIn] = useState(false); // Control hero slide-in animation
  const [pricingSlideIn, setPricingSlideIn] = useState(false); // Control pricing slide-in animation
  ...
  useEffect(() => {
    // Only start animations when onPageLoad is true (after loading screen completes)
    if (onPageLoad) {
      setCanAnimate(true); // Enable animations
      setHeroSlideIn(true); // Start hero slide-in immediately
      setPricingSlideIn(true); // Start pricing slide-in immediately
      ...
      setCanAnimate(false); // Disable animations while loading
      setIsLocked(true);
      setIsLoaded(false);
      setHeroSlideIn(false);
      setPricingSlideIn(false);
    }
  }, [onPageLoad]);
  ...
  return (
    <div className="space-y-0 sm:space-y-10 py-0 relative isolate">
      {/* Hero Section */}
      <section className={`container mx-auto px-4 pt-10 pb-0 sm:pb-0 transition-all duration-1000 ease-out ${heroSlideIn ? "translate-x-0 opacity-100" : "-translate-x-full opacity-0"}`}>
        <div className="container mx-auto text-center max-w-4xl">
          {/* Mobile/Tablet Hero Title - No Animation */}
          <h1 className="lg-1000:hidden text-5xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold leading-tight mb-2 sm:mb-6 text-sp-text-400">
            Unlock
          ...
          <h1
            className="hidden lg-1000:block text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold leading-tight mb-2 sm:mb-6 text-sp-text-400 transition-all duration-1000 ease-out whitespace-nowrap"
            style={{
          ...
          >
            <div className="relative inline-block overflow-hidden text-right" style={{ height: "1.2em", width: "5.5em", marginLeft: "-2em" }}>
              <span
                className={`block absolute bottom-0 left-0 right-0 ${canAnimate ? "transition-all duration-700 ease-in-out" : ""}`}
                style={{
              ...
              <span
                className={`block absolute bottom-0 right-0 ${canAnimate ? "transition-all duration-700 ease-in-out" : ""}`}
                style={{
            ...
            <span className="inline-block mx-2 align-[-.1em] text-sp-white">
              <div className="relative inline-block w-[1.1em] h-[1.1em]">
                <Lock
                  className={`h-[1.1em] w-[1.1em] absolute top-0 left-0 transform ${canAnimate ? "transition-all duration-700 ease-in-out" : ""}`}
                  style={{
                    opacity: isLocked ? 1 : 0,
          ...
          <div
            className="text-lg sm:text-xl md:text-2xl mb-2 sm:mb-8 opacity-90 tracking-tight text-sp-text-200 overflow-hidden relative h-16 transition-all duration-1000 ease-out delay-300"
            style={{
              transform: isLoaded ? "translateY(0) opacity-100" : "translateY(20px) opacity-0",
            }}
          >
      ...
      <section
        id="pricing"
        className={`container mx-auto px-4 pb-12 scroll-mt-64 transition-all duration-1000 ease-out ${pricingSlideIn ? "translate-x-0 opacity-100" : "translate-x-full opacity-0"}`}
      >
        <div className="relative max-w-7xl mx-auto">
          {/* Carousel Layout for all screens under 1000px */}
          <div className="block lg-1000:hidden">
            ...
            <div className="relative overflow-hidden">
              <div className="flex justify-center items-center">
                {/* Left Card (Previous) - Hidden on very small screens */}
                <div className="absolute left-0 sm:left-2 md:left-8 transition-all duration-700 ease-in-out transform scale-50 sm:scale-60 md:scale-75 opacity-60 pointer-events-none hidden sm:block">
                  <div className="w-64 sm:w-80 md:w-96">
                    {currentCard === 0 ? (
                      <PricingCard
                        ...
                        buttonClass="bg-sp-success-600 text-sp-white hover:bg-sp-success-700"
                        onClick={() => {}}
                      />
                    ) : currentCard === 1 ? (
                      <PricingCard
                        ...
                        buttonClass="bg-sp-bg-700 text-sp-text-200 hover:bg-sp-text-600"
                        onClick={() => {}}
                      />
                    ) : currentCard === 2 ? (
                      <PricingCard
                        ...
                        buttonClass="bg-sp-primary-600 text-sp-white hover:bg-sp-primary-700"
                        onClick={() => {}}
                      />
                    ) : (
                      <PricingCard
                        ...
                        buttonClass="bg-sp-secondary-600 text-sp-white hover:bg-sp-secondary-700"
                        onClick={() => {}}
                      />
                    )}
                  </div>
                ...
                {/* Right Card (Next) - Hidden on very small screens */}
                <div className="absolute right-0 sm:right-2 md:right-8 transition-all duration-700 ease-in-out transform scale-50 sm:scale-60 md:scale-75 opacity-60 pointer-events-none hidden sm:block">
                  <div className="w-64 sm:w-80 md:w-96">
                    {currentCard === 0 ? (
                      <PricingCard
                        title="Pro Monthly"
            ...
            <div className="flex justify-center mt-6 space-x-2">
              {[0, 1, 2, 3].map((index) => (
                <button
                  ...
                  className={`w-3 h-3 rounded-full transition-colors duration-200 ${currentCard === index ? "bg-sp-text-400" : "bg-sp-bg-700"}`}
                />
              ))}
            </div>
          </div>

          {/* Desktop Carousel Layout for 1000px+ */}
        ...
        <div className="max-w-6xl mx-auto text-center mt-8">
          <p className="text-sp-text-300">
            * Your <span className="font-extrabold text-sp-text-200">Suite</span>
            <span className="font-normal text-sp-text-300">Preferences</span> subscription is tied to your NetSuite login email and can be updated at any time.
          </p>
          <p className="text-sp-text-300 mt-2">For Enterprise accounts, subscriptions are tied to the logged in user's email domain.</p>
        </div>
      </section>

      {/* Call to Action Section */}
      <section className="bg-gradient-to-br from-sp-bg-900 to-sp-bg-950 text-sp-white py-16 px-4 shadow-xl">
        <div className="container mx-auto text-center max-w-3xl">
          <h2 className="text-4xl font-bold mb-6 text-sp-text-100">Ready to level up your NetSuite UX?</h2>
          <p className="text-xl mb-8 opacity-90 text-sp-text-200">Join other users who are already enjoying a more personalized NetSuite experience.</p>
          <button
            onClick={() => onPlanSelection("Pro Yearly")}
            className="inline-flex items-center px-6 py-4 bg-sp-bg-800 text-sp-white font-bold text-lg rounded-full shadow-lg hover:bg-sp-bg-700 transform hover:scale-105 transition duration-300 ease-in-out focus:outline-none focus:ring-4 focus:ring-sp-text-500 focus:ring-opacity-50"
          >
            <svg
